...
export async function GET(request: NextRequest) {
  ...
  for (const rec of (allRecords ?? [])) {
    ...
    if (!summary[mod]) summary[mod] = { draft: 0, reviewed: 0, validated: 0, rejected: 0, total: 0 };
    const st = rec.validation_status as string;
    if (st in summary[mod]) (summary[mod] as Record<string, number>)[st]++;
    summary[mod].total++;
  }

  // 2. Paginated queue records
  let query = admin
    .from('zafirix_routing_records')
  ...
  // 3. Fetch source document filenames for context
  ...
  const enrichedRecords = (records ?? [] as RoutingRecord[]).map(r => ({
    ...r,
